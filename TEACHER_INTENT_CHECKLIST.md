...
- [x] `pnpm test`: 65개 파일·381/381 통과.
- [x] `pnpm check`: 빌드와 모든 감사 포함 통과.
- [x] MCP in-memory 골든 호출: 실제 첫 문항과 충돌 오류 코드 확인.
- [x] teacher-ui 로컬 HTTP 골든 요청/미리보기: 정답·4개 반영 행·승인 토큰 확인.
...
- 구현 커밋: `14a9958 feat: compile multiplication TeacherIntent`,
  `632cff5 feat: expose TeacherIntent in teacher preview`.
- 테스트 수: 전체 65개 파일·381개 테스트, 집중 9개 파일·65개 테스트.
- QA 보고서 경로: `reports/teacher-intent/latest.md`.
- fallback 활동 수 변화: 20종 → 19종(곱셈 배열 exact preview 등록).
- 남은 수동 확인: 교사용 compose/preview 화면 3문항, fresh canary, 원할 때만 외부
  실생성 1회.
- 알려진 제한: 첫 문항만 맞춤, 맥락 4종, 오개념 1종, 범용 자연어 파서·부분 수정
