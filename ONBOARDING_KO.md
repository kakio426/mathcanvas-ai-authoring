...
## 3. MathCanvas 전용 로그인 창 열기

프로젝트 폴더의 터미널에서 다음 명령을 실행합니다.

```bash
pnpm mathcanvas:login
```

새 Chrome 창이 열리고 MathCanvas 화면으로 이동합니다. 이 창은 평소 사용하는 Chrome과 데이터가 섞이지 않는 MathCanvas 전용 창입니다. 로그인할 때만 이 창을 사용하며, 평소 활동지 생성은 사용자 화면을 점유하지 않는 headless Chrome에서 실행됩니다.
...
4. 터미널에 `PASS MathCanvas 로그인 확인 완료`가 표시되면 전용 Chrome 창을 닫습니다.
...
AI가 학년, 문제 수, 난이도, 조작 방식과 교사용 정답지를 보여 줍니다. 원하는 조건과 다르면 생성 전에 바꿔 달라고 말합니다. 선택한 문제들은 새 활동지 한 개 안에 함께 배치됩니다.
...
완료되면 대화에 새 활동지의 편집 URL이 표시됩니다. 기존 캔버스는 바뀌지 않으며 사용자 화면을 자동으로 가져오지 않습니다.

## 평소 운영

1. `학년 + 수학 주제 + 문제 수`를 말해 추천을 받습니다.
2. 학생이 먼저 판단하고, 조작으로 확인하고, 설명을 고치는 흐름인지 추천안과 정답지를 확인합니다.
3. 조건이 맞으면 `이대로 새 활동지를 만들어 줘`라고 승인합니다.
4. 반환된 편집 URL에서 마지막 문구와 배치를 확인한 뒤 MathCanvas의 기존 공유 기능으로 배포합니다.

저장소를 업데이트한 뒤에는 `git pull --ff-only`을 실행하고 설치 스크립트를 다시 실행해 MCP 등록과 빌드를 최신 상태로 맞춥니다.
...
- `login-required`: `pnpm mathcanvas:login`을 다시 실행하고, 로그인 확인 PASS 뒤 전용 Chrome을 닫습니다.
...
- 로그인 창을 너무 일찍 닫음: `pnpm mathcanvas:login`을 다시 실행하고 터미널의 PASS 안내를 기다립니다.
