...
2022 개정 초등 수학의 공식 분모는 교육부 HWP와 NCIC PDF를 교차 확인한 **121개 성취기준**입니다. 121개 모두 교사용 카탈로그와 71개 교과서 단원에 연결됐지만, canonical ProblemFamily가 있는 성취기준은 24개, `released` family가 닿는 성취기준은 18개(14.9%)뿐입니다. 이 18/121은 활동 reach이며 성취기준의 필수 평가 목표를 모두 다룬다는 뜻이 아닙니다. 현재 reviewed-complete AssessmentTarget set은 2/121, target은 6개이며 그중 live target은 2개라 전체 교육과정 target coverage는 아직 산정하지 않습니다. 새 family 설계가 필요한 정확한 97개는 **24 shared engine · 84 grade-band-safe planning track · 236 expected target outline · W001~W097**로 고정했으며 이는 구현 계획이지 출시 완료 수치가 아닙니다. 최신 수치는 [커버리지 보고서](./reports/curriculum-coverage/latest.md), 12개 대표 격자와 121개 전체 순서는 [전체 실행 보드](./reports/curriculum-execution/latest.md), 97개 구현 계획은 [no-family 실행 보드](./reports/curriculum-execution/no-family-plan.md), Sol 독립 검토 절차는 [SOL_REVIEW_PROMPT.md](./SOL_REVIEW_PROMPT.md)와 [review board](./scripts/curriculum/sol-review-board.json), 완료 정의는 [전 범위 생성 계획](./ELEMENTARY_2022_FULL_COVERAGE_PLAN.md), 확장 규칙은 [ProblemFamily 아키텍처](./docs/PROBLEM_FAMILY_ARCHITECTURE.md)에 있습니다.
...
- 몫과 나머지의 뜻 (`released`): 3~4학년 권장, 묶음으로 나눈 결과를 근거로 몫과 나머지의 뜻 판단·설명
- 회전량으로 각의 크기 판단 (`released`): 4학년 권장, 변의 길이가 아닌 한 변의 회전량을 근거로 각의 크기 판단·설명
- 각의 크기로 삼각형 분류 (`released`): 4학년 권장, 각의 크기를 근거로 직각·예각·둔각삼각형 분류·설명
- 선대칭도형의 대응점 판단 (`released`): 5학년 권장, 대응점과 대칭축 사이의 같은 거리를 근거로 위치 판단·설명
...
- `packages/templates`: 네 영역별 ProblemFamily registry, legacy adapter, blueprint와 유한 variation envelope
