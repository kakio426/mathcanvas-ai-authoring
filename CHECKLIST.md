...
- Evidence/notes:
  ...
  - 같은 canary는 `#top-toolbar`, `#left-toolbar`, `#right-toolbar`, `#bottom-common-toolbar` exact-one bounds로 fixed-safe `x240 y64 976×672`를 다시 파생한다. 이 값은 `student-one-screen-fixed-geometry-v1@1.0.0`의 file/content SHA·8px guard에 exact 결속한다. 안내 3행·제목·응답·네이티브 작업판을 모두 포함한 learner task envelope는 8px guard 안에서 추가 오른쪽 여백 `0.458px`를 가지며 initial/reopened가 동일하다. 7개 lifecycle screenshot은 상태별 exact repo path·PNG SHA·파일 존재를 검증하고 versioned artifact로 보존한다. 31개 source는 답 구조를 읽지 않는 `8-7-8-8`, group reserve는 5묶음 `3×2`·4묶음 `2×2`로 두어 NO01SC 80-unit 조작 크기(`46.426 CSS px`)를 줄이지 않았다. 이 증거는 division reference repair이며 R3 전체·R4 generic one-screen profile·30개 release 완료를 뜻하지 않는다.
  - `research/mathcanvas/division-counting-group-refresh-write-manifest.json`: 로그인 완료 뒤 시작 지시에 결속해 기존 disposable `oEaJt5`만 사용했다. 첫 batch에서 29÷7은 서버 상태를 GET으로 재검증해 PUT 0, 23÷4와 31÷6은 각각 PUT 1, POST/PATCH/DELETE와 unexpected write는 0이었다. 이 batch의 세 canary SHA와 spatial catalog SHA는 아래 current static projection 재검증으로 supersede되며 감사 이력으로만 보존한다.
  - Sol xhigh가 29÷7 reopened 화면이 이전 ② 문구로 되돌아가도 semantic/layout marker만으로 read-only resume를 통과하던 false positive를 발견했다. `division-product-static-v1` projection은 29개 fixed learner-facing object를 current compiled product와 exact hash로 결속하고, transient parent/index와 유한소수 직렬화만 정규화한다. `research/mathcanvas/division-counting-group-29-by-7-recapture-write-manifest.json` 범위에서 current payload PUT 1회, 그 뒤 GET-only save/reopen을 수행했다. current/source/persisted/reopened/second-read static SHA는 모두 `a99abd774dcd...`, full/reopened PNG SHA는 모두 `d28d62dca7b6...`, 최종 canary SHA는 `2e123654bd31...`, unexpected write는 0이다.
  - `research/mathcanvas/division-counting-group-static-projection-backfill-write-manifest.json`: 같은 stale-payload gate를 23÷4·31÷6에도 적용해 각각 PUT 1회 뒤 fresh reopen/second GET을 검증했다. persisted/reopened/second-read static SHA는 23÷4 `66c8fd05ac54...`, 31÷6 `6eca1a980491...`, unexpected write는 0이다. 현재 authoritative canary SHA는 23÷4 `045f8147302d...`, 29÷7 `2e123654bd31...`, 31÷6 `65f158a85e93...`; spatial catalog SHA는 `c678d9043417...`이며 pilot evidence registry와 exact 결속한다.
  - Sol xhigh 최종 재검토는 `OKAY · P0 0 / P1 0 / P2 0`이다. coherent fixed-chrome/profile drift, screenshot path·SHA·containment, top task 침범, manipulated/reopened group chrome 중첩, line-box check-key 위조와 stale learner payload+rehash를 모두 차단했다. static projection은 geometry field에서만 `184.04000000000002↔184.04` 같은 IEEE-754 tail을 정규화하며 `strokeType`, `clickCount`, `fontSize`의 epsilon drift는 exact SHA 차이로 남긴다. 세 canary validator, contract-lab 25/25, typecheck, diff check를 재확인해 이 division reference repair commit gate를 열었다.
...
- Evidence/notes:
  ...
  - pinned source manifest SHA는 `0a57b78ae84b82d7581485fd58747e85cabb7cdefe46321610f4fcee237899f5`, harness content SHA는 `98024ab7fb6ec0a29285a86e131850468b2b2559c849e06a8342315e922c4fcd`이다. 현재 source↔catalog alignment는 exact 27/30이며 25–27번은 Eduitit `분수와 소수`와 catalog `분수` 단원명 차이로 `needs-review`다. catalog availability가 30/30 blocked이므로 design-only prompt 전부 `compileAllowed: false`이며 exact 27건은 `sourceAlignmentReady`로만 구분한다.
