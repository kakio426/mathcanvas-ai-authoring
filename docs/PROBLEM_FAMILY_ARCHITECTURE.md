...
29개 blueprint는 canonical `ProblemFamilyManifest`로 정확히 한 번씩 조회되고,
그중 21개는 기존 release evidence를 보존한 `live-released` 상태다. 이 숫자는
교육과정 완전 커버리지가 아니다. reviewed `AssessmentTarget` 분해는 Phase 2부터
시작하므로 `targetCoverage`는 계속 산정하지 않는다.
...
3. `runtime`: 결정적 prepare 함수, 정답·exact preview, 실제 적용
   `ProblemParameters` projector

이 세 항목을 `ProblemFamilyNativeModule` 하나로 묶어 해당 교육과정 영역 index에
등록한다.
...
generator registry, `ACTIVITY_IDS`, `ACTIVITY_SUPPORT`를 수정하지 않는다. planner는
...
폼으로 투영한다. runtime은 영역 index의 native module을 자동 합친다.
...
- 기존 `templates/src/registry.ts`, `item-generators/registry.ts`,
  `variations/registry.ts`의 수동 목록
...
- family/activity/template ID, generator ID·version, blueprint hash, 교육과정 binding,
  support state가 source와 runtime 사이에서 다르면 시작 시 실패한다.
