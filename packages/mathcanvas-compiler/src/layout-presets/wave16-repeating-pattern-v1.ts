...
export const wave16RepeatingPatternLayoutPreset: LayoutPreset = {
  ...
  tokens: {
    ...
    ...Object.fromEntries(sequence.map((role, i) => [`item.${role}`, { scope: "item" as const, x: 210 + i * 150, y: 205, width: 120, height: 120 }])),
    "item.next-slot-1": { scope: "item", x: 1120, y: 175, width: 188, height: 188 },
    "item.next-slot-2": { scope: "item", x: 1330, y: 175, width: 188, height: 188 },
    ...
    "item.prediction-box": { scope: "item", x: 1860, y: 105, width: 320, height: 64 },
    "item.choice-panel": { scope: "item", x: 1590, y: 195, width: 720, height: 225 },
    "item.pool-label": { scope: "item", x: centeredRowLeft(choiceCenterX, 3, 180, 18) + 50, y: 209, width: 360, height: 34 },
